...
function simulateSeries(start, winPct, games, regressPercent){
  // regressPercent 0..100 -> per-game decay 0..0.02
...
function pathFrom(series){
  // Build grid + line + markers in the svg coordinate space
  while (svg.firstChild) svg.removeChild(svg.firstChild);

  const W = 800, H = 260, P = 28; // padding
...
function labelRank(b){
  return `${b.t}${b.d!=="—" ? " "+b.d:""} (${b.lo}–${b.hi})`;
}

/********** RLTracker scraping (read-only proxy) **********
- Paste a full RLTracker profile URL.
- We read via https://r.jina.ai/http://… (no CORS headers required).
- Parse for the “Ranked Doubles 2v2” block, then pick the biggest 3–4 digit number as MMR.
- Parse Win % if present.
**********************************************************/
async function pullFromRLTracker(url){
  if(!/^https?:\/\/.*tracker\.network\/.*rocket-league/i.test(url)){
    throw new Error("Invalid RLTracker profile URL.");
  }
  ...
  // Try to narrow to the 2v2 section first
  let scope = text;
  const sec = text.match(/(Ranked\s+Doubles\s*2v2[\s\S]{0,1200})/i);
  if (sec) scope = sec[1];

  // Candidate MMR numbers
  const nums = Array.from(scope.matchAll(/\b(\d{3,4})\b/g)).map(m=>+m[1]).filter(n=>n>=-100 && n<=4000);
  const mmr = nums.length ? Math.max(...nums) : null;

  // Win %
  const wrm = scope.match(/Win\s*%[^0-9]{0,6}(\d{1,3}(?:\.\d+)?)/i) || text.match(/Win\s*%[^0-9]{0,6}(\d{1,3}(?:\.\d+)?)/i);
  const winPct = wrm ? Math.max(0, Math.min(100, parseFloat(wrm[1]))) : null;

  if (mmr==null) throw new Error("Couldn’t locate 2v2 MMR on the page.");
...
elFetch.addEventListener("click", async ()=>{
  ...
  try{
    const { mmr, winPct } = await pullFromRLTracker(url);
...
elForm.addEventListener("submit", async (e)=>{
  ...
  // If no MMR but URL provided, try auto-pull now
  if(!elMMR.value && elUrl.value.trim()){
    try{
      const { mmr, winPct } = await pullFromRLTracker(elUrl.value.trim());
      elMMR.value = String(mmr);
      if (winPct!=null) elWin.value = String(winPct);
  ...
  // header label (neutral if no name)
